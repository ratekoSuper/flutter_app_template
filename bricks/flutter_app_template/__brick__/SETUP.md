...
このドキュメントでは、{{app_name}}を使用してアプリ開発を始めるための完全な手順を説明します。
...
`pubspec.yaml`は既に設定済みです。必要に応じて変更してください：

```yaml
name: {{project_name}}  # 既に設定済み
description: "{{description}}"  # 既に設定済み
version: {{version}}  # 既に設定済み
```
...
`android/app/build.gradle.kts`と`android/app/src/main/AndroidManifest.xml`は既に設定済みです。
...
2. **Apple バンドル ID** を入力（ステップ1-2で設定したもの）
   - 例: `{{package_name}}`
3. アプリのニックネーム: 任意（例: `{{app_name}} iOS`）
4. 「アプリを登録」をクリック
...
2. **Android パッケージ名** を入力（ステップ1-3で設定したもの）
   - 例: `{{package_name}}`
3. アプリのニックネーム: 任意（例: `{{app_name}} Android`）
4. 「アプリを登録」をクリック
...
| `android_app_url` | `https://play.google.com/store/apps/details?id={{package_name}}` | String | Google PlayのURL |
...
3. 必要情報を入力：
   ...
   - 名前: {{app_name}}
   - プライマリ言語: 日本語
   - バンドルID: Firebaseで設定したもの（{{package_name}}）
   - SKU: 任意の一意な識別子
4. 「作成」をクリック
...
6. 情報を入力：
   ...
   - **プロダクトID**: 例 `{{package_name}}.premium.monthly`
     - ⚠️ **このIDをメモしてください**
   - **サブスクリプション期間**: 例 1ヶ月
   - **価格**: 設定
7. 「保存」をクリック
...
3. 情報を入力：
   - **プロダクトID**: iOSと同じまたは別のID
     - 例: `{{package_name}}.premium.monthly`
   - **名前**: 商品名
   - **説明**: 商品説明
   - **料金**: 価格設定
4. 「保存」→「有効化」をクリック
...
final Set<String> _productIds = {
  '{{package_name}}.premium.monthly', // ← 実際の商品IDに変更
};
```
...
- [ ] Flutterプロジェクトとして初期化した（`flutter create --platforms=ios,android .`）
- [ ] プロジェクト名を確認した（pubspec.yaml）
- [ ] iOS Bundle IDを変更した
- [ ] Android パッケージ名を確認した
